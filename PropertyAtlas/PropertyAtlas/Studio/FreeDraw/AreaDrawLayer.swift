        ...
        GeometryReader { geo in
            Canvas { ctx, size in
                ...
                if screen.count > 1 {
                    ...
                    if screen.count > 2 { line.addLine(to: screen[0]) }
                    if screen.count > 2 {
                        var fill = Path()
...
struct AreaDrawPanel: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            Text("绘制区域 · \(count) 点").font(Studio.sans(13, .semibold)).foregroundStyle(Studio.on)
            Button("撤销") { onUndo() }.buttonStyle(.tbtn(.ghost)).disabled(count == 0)
            Button("取消") { onCancel() }.buttonStyle(.tbtn(.ghost))
            Button("完成") { onFinish() }.buttonStyle(.tbtn(.primary)).disabled(count < 3)
        }
