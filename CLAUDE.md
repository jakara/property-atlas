...
1. Source files in `reports/` → Python scripts in `scripts/` → JSON in `reports/extracted/`
2. JSON bundled into app
3. `SeedImporter` runs on first launch with progress UI

See "Data Extraction Pipeline" below for `reports/extracted/` schema + provenance.
...
每个脚本独立 idempotent. Python venv: `.venv/` (含 pandas/openpyxl/pdfplumber/jsonschema).
